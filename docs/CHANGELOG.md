...
- **核心機制：合成與進化 (Merge & Evolution)**：
  - **全域合成**：當隊伍中累積足夠數量的同等級同職業勇者時，自動觸發合成（不需要相鄰）。
  - **合成規則**：
    - 3 隻 Lv.1 → 合成 1 隻 Lv.2
    - 4 隻 Lv.2 → 合成 1 隻 Lv.3
    - 5 隻 Lv.3 → 合成 1 隻 Lv.MAX
  - **合成效果**：保留最前方的勇者升級並補滿狀態，其餘勇者消失。
  - **視覺效果**：
    - 顯示 "Lv.X" 文字。
    - 升級時角色發出白光並顯示升級特效。
    - 角色體型隨等級變大 (Lv1=1.0 -> Lv4=1.3)。

- **數值成長系統調整**：
  - **弓箭手**：基礎傷害線性成長 (5 * Level)。
  - **法師**：基礎傷害倍數成長 (3 * 2^(Level-1))。
  - **騎士**：基礎 HP 特殊成長 (2 > 4 > 8 > 12)。

- **騎士技能重構**：
  - 移除「可被攻擊次數」升級。
  - 新增「聖光充能」：每擊殺 10 隻怪物，回復騎士 HP（回復量隨技能等級提升）。

- **道具生成系統優化**：
  - **指定職業道具**：場上固定維持各職業（弓、法、騎）各 2 個指定道具。
  - **隨機道具**：只有當場上指定道具數量滿足時，才會生成隨機道具。
  - **小地圖顯示**：
    - 指定法師：暗藍色
    - 指定弓箭手：暗綠色
    - 指定騎士：鮮黃色
...
- **手機版體驗優化**：
  - **視野自動拉遠**：偵測到螢幕寬度小於 768px 時，自動縮小遊戲畫面（0.75x），擴大可視範圍，解決手機版操控過於緊湊的問題。
  - **UI 適配**：UI 元素保持原大小，僅縮放遊戲世界。

### 核心玩法大改版：合成與進化 (Hero Merge & Evolution)
- **新增勇者合成系統**：
  - 當隊伍中有連續相同職業與等級的勇者時，會自動觸發合成。
  - **合成規則**：Lv1->Lv2 (3隻), Lv2->Lv3 (2隻), Lv3->Lv4 (2隻)。
  - **合併效果**：多餘的勇者往前合併消失，保留的一隻升級並補滿狀態，伴隨金色 "LEVEL UP!" 特效。
  - **視覺顯示**：勇者下方顯示白色 "Lv X" 文字，最高等級顯示 "Lv MAX" (金色)。

### 數值成長與職業調整
- **數值成長 (隨勇者等級提升)**：
  - **弓箭手**：基礎傷害線性成長 (Lv1=5, Lv2=10, Lv3=15, Lv4=20)。
  - **法師**：基礎傷害倍數成長 (Lv1=3, Lv2=6, Lv3=12, Lv4=24)。
  - **騎士**：基礎 HP 倍數成長 (Lv1=2, Lv2=4, Lv3=8, Lv4=16)。
- **騎士機制重構**：
  - **移除升級**：移除「可被攻擊次數」升級選項。
  - **新增技能**：「聖光充能」- 每擊殺 10 隻怪物，回復騎士 HP（回復量隨升級提升，Lv1 回 1 點）。
  - **基礎調整**：騎士基礎 HP 從 1 改為 2，並隨等級倍增。
...
- **平衡調整**：
  ...
  - **怪物移動速度**：等級越高移動速度越快，等級 1 = 100% 基礎速度，等級 8 = 130% 基礎速度（每級約 +4.3%），差距適中。
  - **玩家升級難度**：降低升級所需經驗值，`baseExp` 從 80 降至 50，`expMultiplier` 從 1.3 降至 1.2，讓升級更容易。
  - **技能等級顯示**：技能初始等級改為 0，顯示為 "Lv 0" 而非 "Lv 1"，更符合邏輯。
  - **騎士 hitPoints 升級**：當騎士「可受攻擊次數」升級時，場上所有現有騎士的 hitPoints 會增加對應的增量（而非補滿），讓升級更有意義。
  - **遊戲平衡調整**：
    - **怪物強度提升**：為了應對勇者合成後的戰力提升，大幅強化 Lv2 以上怪物的各項數值。
    - **怪物血量**：成長幅度從每級 +30 提升至 +45，並引入成長係數（每級額外 +15%），讓高等級怪物血量呈指數增長。
    - **怪物傷害**：成長幅度從每級 +10 提升至 +15。
    - **怪物速度**：Lv1-4 每級增速提升至 5%，Lv5+ 每級增速提升至 8%。
    - **怪物出現時機**：延後高等級怪物出現時機，讓玩家有更多時間適應和升級。
    - **升級難度**：進一步降低升級所需經驗值，`baseExp` 從 40 降至 25，`expMultiplier` 從 1.15 降至 1.1，讓升級更快更容易。
    - **怪物經驗值**：高等級怪物經驗值改為非線性成長，等級越高經驗值成長越快，鼓勵玩家挑戰高等級怪物。
    - **怪物強度提升**：增加高等級怪物的血量和傷害，`hpPerLevel` 從 20 增加到 30，`damagePerLevel` 從 7 增加到 10，讓高等級怪物更具挑戰性。
    - **弓箭手爆炸傷害調整**：爆炸傷害從每級 +5 降低到 +2，避免過強。
    - **高等級怪物強化**：等級 5 以上的怪物速度更快（每級額外增加 7% 速度），血量更多（每級額外增加 50% 血量），讓高等級怪物更具挑戰性。
  - **騎士爆炸機制調整**：
    - **受傷爆炸**：將騎士「死亡爆炸」改為「受傷爆炸」，每次扣血時觸發，傷害降低一半（從 +10/級 降至 +5/級），避免過強。
    - **受傷爆炸範圍提升**：爆炸範圍從每級 +40px 提升至 +70px，讓受傷爆炸更具威脅性。
  - **弓箭手平衡調整**：
    - **瞄準與射擊距離**：瞄準距離調整為 8 格（320 像素），射擊距離與瞄準距離相同，不再無限遠。
    - **傷害調整**：弓箭基礎傷害從 10 減半為 5，降低弓箭手整體強度。
    - **爆炸範圍提升**：爆炸範圍從每級 +30px 提升至 +50px。
    - **爆炸傷害調整**：爆炸傷害從每級 +5 降低到 +2，避免過強。
  - **能力類型系統調整**：
    - **上限調整**：能力類型上限從 10 種降低到 8 種，讓玩家需要更謹慎選擇。
    - **升級選項加權隨機系統**：已解鎖未滿級選項出現機率提高 50%（權重 1.5），未解鎖選項權重 1.0，滿級選項權重 0.5，讓已解鎖選項更容易出現但不會完全壟斷。
