...
const ENEMY_SPEED = 1.2;    // 敵人基礎逐幀速度 (px/frame)，實際速度會根據等級調整（等級 1 = 100%，等級 8 = 130%）
...
const ARROW_DAMAGE = 5;     // 弓箭傷害（從 10 減半為 5）
...
- **合成機制**：
  - 當隊伍中有連續相同職業與等級的勇者時，會自動觸發合成：
    - **Lv1 -> Lv2**：需要 3 隻連續 Lv1
    - **Lv2 -> Lv3**：需要 2 隻連續 Lv2
    - **Lv3 -> Lv4 (Max)**：需要 2 隻連續 Lv3
  - 合成後，多餘的勇者會消失（往前合併），並保留最前面的一隻升級為下一等級。
  - 合成時會補滿該勇者的血量（特別是騎士）。
  - **視覺**：勇者下方顯示白色文字等級 (如 Lv 2)，Lv 4 顯示 "Lv MAX"。

- **數值成長 (隨勇者等級提升)**：
  - **弓箭手**：基礎傷害隨等級線性成長 (`5 + (Level-1)*5`)。Lv1=5, Lv2=10, Lv3=15, Lv4=20。
  - **法師**：基礎傷害隨等級倍數成長 (`3 * 2^(Level-1)`)。Lv1=3, Lv2=6, Lv3=12, Lv4=24 (光環傷害)。
  - **騎士**：基礎 HP 隨等級倍數成長 (`2 * 2^(Level-1)`)。Lv1=2, Lv2=4, Lv3=8, Lv4=16。

### 4.6 職業技能系統

#### 弓箭手 (Archer)
- **攻擊方式**：每 `ARCHER_COOLDOWN` 鎖定最近敵人（距離限制：`ATTACK_RANGE`，約 8 格）
- **投射物**：發射箭矢，速度可升級（基礎 `PROJECTILE_SPEED`），傷害隨勇者等級提升。
- **射擊距離限制**：箭矢射擊距離與瞄準距離相同（320 像素），超過距離後自動消失，不再無限遠
- **升級效果**：
  - **弓箭數量**：每次攻擊可發射多支箭矢（最大 Lv5，從 Lv3 上修）
  - **射擊速度**：箭矢飛行速度提升，同時減少射擊冷卻時間（最大 Lv8，從 Lv10 下修，冷卻時間最多減半）
  - **箭矢爆炸**：命中怪物後對周圍敵人造成傷害（範圍每級+50px，傷害每級+2，最大 Lv5）
  - **必殺 (Critical)**：5%/級機率造成 2 倍傷害，觸發時顯示金色大型文字與特效（最大 Lv10）
- **視覺**：綠色背景，弓箭圖示
- **碰撞**：箭矢不能穿透，擊中敵人後立即移除
- **爆炸效果**：如果升級了爆炸效果，箭矢命中敵人時會對範圍內所有敵人（包括被命中的敵人）造成傷害，並顯示綠色爆炸特效。

#### 法師 (Mage)
- **攻擊方式**：對光環範圍內敵人造成持續傷害（傷害間隔：`AURA_HIT_INTERVAL` = 200ms），傷害隨勇者等級提升。
- **傷害範圍**：考慮光環線條寬度和敵人大小，確保視覺範圍與傷害範圍一致
- **升級效果**：
  - **施法範圍**：光環範圍增加（基礎 `AURA_RADIUS`，最大 Lv8）
  - **施法傷害**：光環傷害增加（基礎傷害隨勇者等級成長，此升級額外增加固定傷害，每次升級 +3，最大 Lv5）
...
#### 騎士 (Knight)
- **防禦機制**：敵人碰撞時優先由騎士代為犧牲
- **hitPoints 系統**：騎士有可被攻擊次數 (HP)，隨勇者等級倍數成長 (Lv1=2, Lv4=16)。
  - 每次被撞減少 1 點 hitPoints
  - 根據剩餘 hitPoints 百分比調整透明度（100% 時完全不透明，0% 時 30% 透明）
  - hitPoints 歸零時移除騎士
- **守護機制**：若隊伍仍有騎士，敵人撞到任何隊員（除隊長外）都會由最先的騎士代為犧牲
- **無敵機制**：騎士死亡後觸發全隊無敵（每級 +1秒，最大 Lv5）。
- **升級效果**：
  - **聖光充能 (New!)**：每擊殺 10 隻怪物，回復騎士 HP（回復量隨等級提升，最大 Lv5）。
  - **死亡後增加隊伍長度**：騎士死亡時隊伍長度增加（可升級，最大 Lv3）
  - **受傷爆炸**：騎士受傷時爆炸範圍 +70px/級，傷害 +5/級（最大 Lv8）。每次扣血時觸發，傷害比死亡爆炸降低一半，避免過強。
...
- **能力類型限制**：
  - 每輪遊戲最多可解鎖 8 種不同的能力類型（從 10 降低到 8）
  - 能力類型計數器顯示在遊戲界面（「能力類型: X/8」）
...
- **升級選項加權隨機系統**：
  ...
  - 滿級選項：權重 0.5（降低出現機率）
  - 優先從不同職業中選擇，確保多樣性
  - 讓已解鎖選項更容易出現，但不會完全壟斷選項

### 4.9 怪物等級系統
- **等級範圍**：怪物有 1-8 級（對應 `mob_1.png` 到 `mob_8.png`），等級越高越強
- **屬性計算**：
  - **血量**：等級 1-4 正常計算，等級 5 以上每級額外增加 50% 血量
    - 等級 1：20 HP
    - 等級 4：110 HP（20 + 3×30）
    - 等級 5：155 HP（110 + 1×45）
    - 等級 8：290 HP（110 + 4×45）
  - **傷害**：`baseDamage + (level - 1) * damagePerLevel`（等級 1：35 傷害，等級 8：105 傷害，每級增加 10 傷害）
  - **速度**：等級 1-4 每級約 +4.3%，等級 5 以上每級額外增加 7% 速度
    - 等級 1：100% 基礎速度
    - 等級 4：113% 基礎速度
    - 等級 5：120% 基礎速度
    - 等級 8：150% 基礎速度
