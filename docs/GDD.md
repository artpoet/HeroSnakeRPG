...
9. **全球排行榜**：
   - 透過 Firebase Firestore 紀錄玩家名稱、最長隊伍長度、上傳日期。
   - Game Over 後玩家可輸入暱稱並上傳分數。
   - 主畫面右側即時顯示前 10 名成績，使用 onSnapshot 即時更新。
...
3. **Game Over**：半透明遮罩、顯示文字、「重新開始」按鈕，以及本局最長隊伍 & 總擊殺統計。
4. **血條顯示**：
   - 隊長與敵人受到傷害後，頭頂浮現漸層血條（綠→紅），血滿時不顯示，避免遮擋地圖。
   - 血條高度僅 3~4px，確保視覺輕量。
5. **載入畫面**：進入頁面時顯示資產載入條，完成後自動開始遊戲。
6. **右側指南**：常駐圖文面板，介紹各職業、敵人與道具，協助玩家理解角色定位。
7. **全球排行榜面板**：與指南共用右側欄位，以暗色卡片展示前 10 名成績並標示「即時同步」狀態。
